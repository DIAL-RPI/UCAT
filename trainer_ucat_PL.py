...
def train(train_loader_source, train_loader_source_batch, train_loader_target, train_loader_target_batch, 
          model, learn_cen, learn_cen_2, criterion_cons, optimizer, itern, epoch, new_epoch_flag, src_cs, 
          args, Th_classwise, keys=None, lab_generator=None):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    top1_source = AverageMeter()
    losses = AverageMeter()
    losses_clus = AverageMeter()
    losses_trade = AverageMeter()
    ...
    try:
        ...
        (input_target, target_target, _) = train_loader_target_batch.__next__()[1]
    # if keys is not None and keys.find('T') != -1: # attack include target domain
    #     input_target_att = attack_TRADES(model=model, class_label=target_target, config=args)
    ...
    input_target_var = input_target_var.cuda()
    # print(input_target_var.device)
    ...
    if args['src_cls']:
        ...
        # if keys is not None and keys.find('S') != -1: # attack include target domain
        #     input_source = attack_PGD(model=model, x_natural=input_source, class_label=target_source, config=args, train_=True)
    ...
    if keys is not None and keys.find('T') != -1: # attack include target domain
        input_target_att = attack_TRADES(model=model, x_natural=input_target_var, config=args)
    input_target_att_var = Variable(input_target_att)
    input_target_att_var.cuda()

    feature_src = f_s.detach()
    feature_tgt,_,logits_nat = model(input_target_var)
    feature_tgt_att,_,logits_adv = model(input_target_att_var)

    '''--- pseudo labeling ---'''
    _,_,class_logits = lab_generator(input_target_var)
    prob = nn.functional.softmax(class_logits)
    pred = class_logits.data.max(1, keepdim=True)[1]
    pred_ = pred.squeeze()
    prob_max, _ = torch.max(prob,1) ### confidence of each sample
    prob_th = Th_classwise[pred_] ### confidence threhold
    prob_th, prob_max = prob_th.cuda(), prob_max.cuda()
    prob_bool = prob_max>prob_th
    row_index = torch.where(prob_bool==True)[0]
    lab_tgt = pred[row_index][:,0]         # cover lab_tgt
    feature_tgt = feature_tgt[row_index]   # cover feature_tgt_
    feature_tgt_att = feature_tgt_att[row_index] # cover feature_tgt_

    '''--- total losses ---'''
    ### SAA loss
    Loss_Contrastive = ContrastiveLoss(margin=25)
    loss_clus_t = Loss_Contrastive(feat1=feature_src, feat2=feature_tgt, lab_1=target_source_var, lab_2=lab_tgt)
    loss_clus_tat = Loss_Contrastive(feat1=feature_src, feat2=feature_tgt_att, lab_1=target_source_var, lab_2=lab_tgt)
    loss_clus = (loss_clus_t + loss_clus_tat)/10
    losses_clus.update(loss_clus.data.item(), input_target.size(0))

    ### TRADES loss
    criterion_kl = nn.KLDivLoss(size_average=False)
    batch_size = len(logits_nat)
    loss_trade = (1.0 / batch_size) * criterion_kl(F.log_softmax(logits_adv, dim=1), F.softmax(logits_nat, dim=1))
    losses_trade.update(loss_trade.data.item(), input_target.size(0))


    model.zero_grad()
    Loss_tot = loss + loss_trade + loss_clus
    Loss_tot.backward()
    optimizer.step()
    ...
    if itern % args['print_freq'] == 0:
        print('Train - epoch [{0}/{1}]\t'
              ...
              'TRADE {loss_trade.val:.4f} ({loss_trade.avg:.4f})\t'
              'SAA {loss_clus.val:.4f} ({loss_clus.avg:.4f})\t'.format(
               epoch, 
               args['epochs'], 
               ...
               s_top1=top1_source, 
               loss=losses,
               loss_trade=losses_trade,
               loss_clus=losses_clus))
...
class ContrastiveLoss(nn.Module):
    ...
    def compute_paired_mask(self, lab_A, lab_B):
        x = lab_A.unsqueeze(1)
        y = lab_B.unsqueeze(0)
        mask = (x == y).type(torch.uint8)
        mask = mask.type(torch.float32)
        return mask.cuda()
        
    def forward(self, feat1, feat2, lab_1, lab_2): 
        # intra-class: flag=0;    inter-class: flag=1
        dist2_mat = self.compute_paired_dist(feat1, feat2)
#         print(torch.flatten(dist2_mat)[:30])
        lab2_mask_intra = self.compute_paired_mask(lab_1, lab_2)
        lab2_mask_inter = torch.ones(lab2_mask_intra.size()).cuda()-lab2_mask_intra
        
        loss_pos = torch.sum(dist2_mat*lab2_mask_intra)/torch.sum(lab2_mask_intra)
        loss_neg = torch.pow(F.relu(self.margin - (dist2_mat*lab2_mask_inter+self.eps).sqrt()), 2)
        loss_neg = loss_neg.sum() / (loss_neg > 1e-6).sum()
        
#         intra_dist = torch.sum((dist2_mat*lab2_mask_intra).sqrt())/torch.sum(lab2_mask_intra)
#         inter_dist = (dist2_mat*lab2_mask_inter).sqrt()
#         inter_dist = inter_dist.sum()/(inter_dist>1e-6).sum()
#         print("intra", intra_dist)
#         print("inter", inter_dist)
        
        loss_contrastive = 0.5 * torch.sum(dist2_mat*lab2_mask_intra)/torch.sum(lab2_mask_intra) +\
                    0.5 * torch.sum(torch.pow(F.relu(self.margin - (dist2_mat*lab2_mask_inter+self.eps).sqrt()), 2))/torch.sum(lab2_mask_inter)
...
def validate(val_loader, model, criterion, epoch, args, adv_attack=False):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target, _) in enumerate(val_loader):
        ...
        if adv_attack is True: 
            input = attack_PGD(model=model, x_natural=input, class_label=target, config=args, train_=False) # validation
    ...
    if adv_attack is True:
        print('Robness on T test set - Prec@1 {top1.avg:.3f}, Prec@5 {top5.avg:.3f}'.format(top1=top1, top5=top5))
    else:
        print('NormAcc on T test set - Prec@1 {top1.avg:.3f}, Prec@5 {top5.avg:.3f}'.format(top1=top1, top5=top5))
    
    return top1.avg

    
def validate_compute_cen(val_loader_target, val_loader_source, model, criterion, epoch, args, compute_cen=True, keys=None):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    source_features = torch.cuda.FloatTensor(len(val_loader_source.dataset.imgs), 2048).fill_(0) # tot num of imgs
    ...
    if compute_cen:
        for i, (input, target, index) in enumerate(val_loader_source): # the iterarion in the source dataset
            target = target.cuda(non_blocking=True)
            if keys is not None and keys.find('S') != -1: # attack include target domain
                input = attack_PGD(model=model, x_natural=input, class_label=target, config=args, train_=False)
            input_var = Variable(input)
            ...
            target_.scatter_(1, target.unsqueeze(1), torch.ones(output.size(0), 1).cuda()) # one-hot coding
    ...
    for i, (input, target, index) in enumerate(val_loader_target): # the iterarion in the target dataset
        # if keys is not None and keys.find('T') != -1: # attack include target domain
        #     input = attack_TRADES(model=model, x_natural=input, config=args)
        ...
        if compute_cen: # compute target class centroids, no target_, use pred_
...
def accuracy(output, target, topk=(1,)):
    ...
    res = []
    for k in topk:
#         correct_k = correct[:k].view(-1).float().sum(0, keepdim=True) # sum up correctness of topk values
        correct_k = correct[:k].reshape(-1).float().sum(0, keepdim=True) # sum up correctness of topk values
